...
class ElectronBeam:
    ...
    """
    def __init__(self, b_d, r_d, b_c, r_c, t_c):
        """
    ...
    @staticmethod
    def characteristic_potential(cur, e_kin):
        """
        Returns the characteristic potential due to spacecharge in V

        Input Parameters
        cur - electron beam current in A
        e_kin - electron energy in eV
        """
        v = ElectronBeam.electron_velocity(e_kin)
        return cur / (4 * PI * EPS_0 * v)

    def herrmann_radius(self, cur, e_kin):
        """
    ...
    def space_charge_correction(self, r, cur, e_kin):
        ...
        while (sc_on_ax_new - sc_on_ax_old)/sc_on_ax_new > 1e-6: # Check relative difference
            ...
            r_e = self.herrmann_radius(cur, corr_e_kin)
            phi_0 = ElectronBeam.characteristic_potential(cur, e_kin)
